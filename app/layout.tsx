...
export default function RootLayout({
    ...
    return (
        <html lang="en">
        <body
            className={`${livvic.className} ${robotoSerif_init.variable} gap-2  flex w-screen px-10 bg-green py-2 items-center justify-center h-screen  flex-col `}>
        <NextTopLoader/>

        <span className="uppercase text-sm opacity-60 text-white  hover:opacity-100 duration-300">bromuno hangout</span>

        <main
            className="relative flex bg-white rounded-2xl w-full h-full max-h-[900px] flex-1 max-w-screen-2xl overflow-clip overflow-y-scroll  scrollbar-hide">
            <StoreProvider>
                <LoadingOverlay/>
                <FormContextProvider>
                    <LeftBar/>
                    <div
                        className="h-full relative scrollbar-hide flex flex-col flex-1 px-4 bg-gray-50 overflow-hidden overflow-y-scroll  ">
                        <Title/>
