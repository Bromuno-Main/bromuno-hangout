...
export function Answer({ answer, setAnswer, id }: propType) {
  ...
  return (
    <div
      className={`fixed z-50   lg:left-[16%] lg:top-[7vh] left-0 top-0 rounded-lg  flex flex-col bg-white shadow-lg lg:h-[85vh] lg:w-[661px] w-full h-full overflow-hidden my-auto   overflow-y-scroll scrollbar-hide`}
    >
      {/* close icon */}
      <div
        onClick={() => {
                ...
                <div className="border-stone-300 border-b-1 py-6 ">
                  <div className="px-5 flex flex-col  gap-3">
                    ...
                    <div className="flex gap-2 items-center justify-start">
                      ...
                      <p className="text-sm group-focus">
                        Leave an upvote if you found this helpful
                      </p>
                    </div>
                    {/* input----- */}
                    <div className="h-full w-full flex flex-col relative mb-4  group ">
                      <input
                        value={commentText}
                        onChange={(e) => setCommentText(e.target.value)}
                        type="text"
                        placeholder="Leave a comment"
                        className="border p-2 focus:outline-none focus:border-b-1 "
                      />

                      {/* Parent with focus-within */}
                      <div className="relative bg-pink-600 focus-within:opacity-100 focus-within:translate-y-0 opacity-0 translate-y-2 transition-all duration-300">
                        <button
                      ...
                      .map((post, index) => {
                        return (
                          <div
                            key={index}
                            className={`${
                              index === 0 ? "border border-[#6AC5AE]" : "border"
